...
class MyDataHandler:
  ...
  def get_UD_Values(self, url):
    train_x=[]
    train_y=[]

    # Get the file name of the url and create a local copy
    filename = Path(url).stem
    path = tf.keras.utils.get_file("/content/" + filename, url)
    ...
    for sentence in sentences:
      ...
      for token in sentence:
        ...
        if (token['form'] != "_" and token['upos'] != "_"):
          new_sentence.append(token['form'])
  ...
  # Gets the data from a url in a format ready to be used by the model
  def get_Procesed_Data(self, url, replaceOOV, showSamples):
    # Reads the file in universal dependecies format and returns a tuple with inputs and outputs
    (train_x, train_y) = self.get_UD_Values(url)
    ...
    # Get the values in a one hot encode
    binary_outputs = self.get_OneHotEncoding_Values(padded_outputs)
  ...
  # Gets the data from a url in a format ready to be used by the advance model
  def get_char_Procesed_Data(self, url, replaceOOV, showSamples):
    # Reads the file in universal dependecies format and returns a tuple with inputs and outputs
    (train_x, train_y) = self.get_UD_Values(url)
  ...
  # Format a text to an acceptable value for prediction
  def get_Test_Data(self, text_to_predict):
    # Splits the text to words and puntuation
    text_to_predict = text_to_predict.replace('.',' .').replace(',',' ,').replace('?',' ?').replace(':',' :')
    text_to_predict = text_to_predict.split()

    # Get the text in numbers
    (coded_text_to_predict, x) = self.get_Tokenized_Values([text_to_predict], self.word_tokenizer)

    # returns a tuple with the padded numbers and the original lenght of the splitted text
    return (self.get_Padded_Values(coded_text_to_predict), len(text_to_predict))

  # Format a text to an acceptable value for advanced prediction
  def get_char_Test_Data(self, new_text_to_predict):
    text_to_predict = []

    # Splits the text to words and puntuation (to take each word separated)
    for ttp in new_text_to_predict:
      ttpc = ttp.replace('.',' .').replace(',',' ,').replace('?',' ?').replace(':',' :')
      ttpc = ttpc.split(' ')
      text_to_predict.append(ttpc)
